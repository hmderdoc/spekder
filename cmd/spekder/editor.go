					...
					case edFile:
						...
						case "PLAYTEST":
							if gm.FatalIssues(gm.ValidateMap(m)) {
								...
								sess := newOfflineOnMap(idx, offlineBots, gm.EffectiveMode(m), s.difficulty, s.aimAssist, playerName, gm.SelectColors[0], gm.BodyTank)
								quit, _ := playMatch(w, cols, rows, rows3d, rnd, ip, sess, dropfile, "playtest", m.Name, false, false, nil)
								sess.close()
								gm.Maps = gm.Maps[:idx]
								if quit {
