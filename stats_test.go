...
func TestScoreAndRecord(t *testing.T) {
	...
	r := matchResult{Mode: 0, ModeName: "DEATHMATCH", MapName: "X", Vehicle: "HUNTER", Player: "P",
	...
	if st.PerMode["DEATHMATCH"].Games != 1 || st.PerVehicle["HUNTER"] != 1 {
	...
	if favoriteMode(&st) != "DEATHMATCH" || favoriteVehicle(&st) != "HUNTER" {
